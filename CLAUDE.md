...
- **Analiza la foto del producto con Gemini (no Claude)**: nodo `Tiene Imagen` (IF, comprueba `Boolean($binary['Foto_del_producto'])`) → si hay foto, rama TRUE va a `0 - Descripcion Visual` (**Google Gemini**, resource `image`/operation `analyze`, modelo `models/gemini-3.6-flash` (gemini-2.5-flash quedó descatalogado para cuentas nuevas), credencial `googlePalmApi`) que la describe a fondo (aspecto, colores, cómo se usa, qué genera efecto WOW, cómo grabarla); si no hay foto, la rama FALSE salta directa a `1 - Conciencia de Mercado` sin gastar nada. Se eligió Gemini para esta parte porque es más barato que Claude y mejor leyendo texto denso en capturas (precio/valoraciones/envío); los 6 pasos de investigación siguen en Claude. Los prompts 1 y 2 leen la descripción con `try/catch` (por si la rama sin imagen no ejecutó el nodo) extrayendo `content.parts[].text` (formato de salida de Gemini, distinto al `merged_response` de Anthropic).
- **13 nodos**: Form Trigger → `Tiene Imagen` (IF) → `0 - Descripcion Visual` (Gemini, condicional) → 6 nodos en cascada → `7 - Resumen Final Telegram` (condensa las 6 salidas en UN solo mensaje HTML ≤3500 caracteres, secciones en MAYÚSCULAS+negrita) → `Preparar Mensaje Telegram` (Code, trunca por seguridad si se pasa) → `Telegram - Enviar Research` (manda al chat `541043415`) → si falla el HTML, reintenta por la rama de error con `Correct errors` (mismo patrón que FILTRO).
- Credencial Gemini `Google Gemini(PaLM) Api account` (id `q2RaU4TfIgraQJDC`) creada por el usuario en n8n con una API key gratuita de Google AI Studio. ✅
- ⚠️ **TEMPORAL — todo el workflow corre en Gemini, no en Claude**: la cuenta de Anthropic del usuario está sin saldo (`Your credit balance is too low`), así que mientras no cargue crédito en https://console.anthropic.com/settings/billing, los 7 nodos (1-7, antes Anthropic) se cambiaron a **Google Gemini** (`@n8n/n8n-nodes-langchain.googleGemini`, typeVersion 1.1, resource "text"/operation "message", modelo `models/gemini-3.6-flash`, credencial `googlePalmApi`, misma que el nodo de visión) para poder seguir probando gratis. Mismo patrón de nodo suelto (no AI Agent), mismo `options.includeMergedResponse: true` que Anthropic (el código de después no tuvo que tocarse, el campo se llama igual: `merged_response`). Búsqueda web real vía `builtInTools.googleSearch: true` (activada por defecto en el nodo, viene true) en `1 - Conciencia de Mercado`, `2 - Competidores` y `3 - Psicografia RMBC`; desactivada (`false`) en los pasos de síntesis 4, 5, 6 y 7.
  - **Cuando el usuario cargue crédito en Anthropic, revertir 1-7 a `@n8n/n8n-nodes-langchain.anthropic`** (resource "text"/operation "message", credencial `anthropicApi` id `gOgpOMeVH3GoNfho`, modelo `claude-opus-5-5` para 1-6 y `claude-sonnet-5` para 7, `options.webSearch: true` en 1-3, sin `temperature` — ver regla 8) para volver a la calidad de razonamiento de Opus. El nodo de visión (`0 - Descripcion Visual`) se queda en Gemini de forma permanente (decisión de coste, no temporal).
...
6. **Elección de modelo Claude por tipo de workflow**: `claude-opus-5-5` en los pasos de razonamiento/estrategia profunda de workflows que se lanzan pocas veces (ej. Estudio de Mercado: los 6 prompts de investigación). `claude-sonnet-5` en workflows de alta frecuencia donde importa velocidad/coste (FILTRO, que se lanza por cada producto) y en pasos mecánicos dentro de cualquier workflow (visión de imagen, condensar/formatear el mensaje final de Telegram). ⚠️ Excepción temporal: Estudio de Mercado está corriendo en Gemini (no Claude) mientras la cuenta de Anthropic no tenga saldo — ver nota en la sección de Estudio de Mercado arriba, revertir a Claude/Opus en cuanto haya crédito.
7. **Formato de TODOS los mensajes de Telegram (FILTRO, RESEARCH, GUION, GANCHO)**: un único mensaje por resultado, nunca trocear en varias partes. Cada sección diferenciada con su título EN MAYÚSCULAS y en negrita (`<b>TÍTULO</b>`), bullets cortos y accionables debajo, fácil de leer de un vistazo. Límite real de Telegram: 4096 caracteres — si el contenido no cabe, no se trocea en mensajes, se **condensa** (paso extra de IA que resume a lo esencial) para que siga siendo un solo mensaje bien estructurado. Así está montado RESEARCH (nodo `7 - Resumen Final Telegram` condensa las 6 salidas en un único mensaje ≤3500 caracteres antes de mandarlo).
8. **Nunca poner `temperature` en las opciones de los nodos Anthropic** (ni el nodo estándar `@n8n/n8n-nodes-langchain.anthropic` ni el sub-nodo `lmChatAnthropic`). Los modelos actuales (`claude-sonnet-5`, `claude-opus-5-5`) lo rechazan con `invalid_request_error: "temperature" is deprecated for this model` y el nodo corta la ejecución al instante sin avisar (parece que "no avanza" pero en realidad ya murió en el primer paso). Ya se quitó de FILTRO y de Estudio de Mercado — si se crea un nodo Anthropic nuevo (GUION, GANCHO), no añadir ese parámetro.
